...
DomUtils = new function ()
{
	...
	thisRef.iterate = function (args)
	{
		...
		/// </param>
		if (args == null || args.node == null || args.node.childNodes == null || typeof (args.task) != "function") { return; }
	...
	thisRef.findUp = function (elt, params)
	{
		...
		do
		{
			...
			if (styles != null && elt.style != null)
			{
				...
				{
					if (elt.style[st] == styles[st]) { return elt; }
				}
			}
			...
			if (attributes != null && elt.attributes != null)
			{
				...
				{
					if (elt.getAttribute(st) == attributes[st]) { return elt; }
				}
			}
		}
	...
	thisRef.smoothScroll = function (el, steptime, x, speedx, axelx, y, speedy, axely, onScrollEnd)
	{
		...
		if (x != null && x > 0 || y != null && y > 0)
		{
			...
			speedx *= axelx;
			speedy *= axely;
			setTimeout(function () { thisRef.smoothScroll(el, steptime, x, speedx, axelx, y, speedy, axely, onScrollEnd); }, steptime);
		}
	...
	thisRef.toggleClassName = function (element, className, remove)
	{
		if (!element || !element.className || element.className.constructor != String || !className || className.constructor != String)
			return;
		var existClasses = ArrayHelper.where(element.className.split(' '), function (item) { return item.length > 0; });
...
SizeHelper = new function ()
{
	...
	thisRef.position = function (elem)
	{
		...
		return { left: thisRef.left(elem), top: thisRef.top(elem), width: thisRef.width(elem), height: thisRef.height(elem) };
	};
};
...
CallHandlers = function ()
{
	...
	{
		if (typeof (handler) == "function") { handlers.push({ handler: handler, state: state }); }
	};
	...
	{
		instance.get_handlers = function() { return handlers; };
	}
};
...
ArrayHelper = new function ()
{
	...
	{
		if (array == null || array.length == null || !(array.length > 0) || typeof (qualifier) != "function") { return -1; }
	...
	thisRef.indexOf = function (array, element, comparer)
	{
		...
		if (typeof (comparer) != "function") { comparer = function (a, b) { return a === b; }; }
...
TimeoutHelper = function (timeoutInterval, callbackFunction)
{
	...
	instance.call = function ()
	{
		...
		_timeout = setTimeout(function () { _callback.apply(callThis, callParameters); instance.inCall = false; }, _timeoutInterval);
	};
};
...
ErrorHelper = new function ()
{
	var thisRef = this;

	thisRef.logError = function (er, component)
	{
		Sogo.CommonControls.Services.LogService.LogError(er, component);
	};

	thisRef.logWarning = function (er, component)
	{
		Sogo.CommonControls.Services.LogService.LogWarning(er, component);
	};

	thisRef.logDebug = function (er, component)
	{
		Sogo.CommonControls.Services.LogService.LogDebug(er, component);
	};

	thisRef.logStringError = function (er, component)
	{
		Sogo.CommonControls.Services.LogService.LogError({ message: er, fileName: "", lineNumber: "0", name: "UnhandledError" }, component);
	};
};
...
var DigitInputHelper = new function ()
{
	var thisRef = this;
	...
	var defaultParams = {
		...
		max: 100,
		allowEmpty: false,
		decimalSeparator: ".", // separator between integer and fraction parts
		groupSeparator: "," // separator between thousands
	};

	thisRef.detach = function (controlId)
	{
		var control = ge(controlId);

		if (control == null || control.tagName != "INPUT" || control.type != "text")
			return;

		if (control.params == null)
			return;

		EventHelper.remove(control, "keyup", keyUpHandler);
		EventHelper.remove(control, "keydown", keyDownHandler);
		EventHelper.remove(control, "keypress", keyPressHandler);
		EventHelper.remove(control, "blur", blurHandler);
		EventHelper.remove(control, "change", changeHandler);
	};

	thisRef.attach = function (params)
	{
		if (params == null)
			return;
		...
		if (isStringEmpty(controlId) || ((control = ge(controlId)) == null) ||
			control.tagName != "INPUT" || control.type != "text")
		{
			return;
		}

		EventHelper.add(control, "keyup", keyUpHandler);
		EventHelper.add(control, "keydown", keyDownHandler);
		EventHelper.add(control, "keypress", keyPressHandler);
		EventHelper.add(control, "blur", blurHandler);
		EventHelper.add(control, "change", changeHandler);

		params = ObjectsHelper.merge(params, defaultParams);
		control.params = params;
	};

	function applyParams(evt)
	{
		var input = EventHelper.getEventTarget(evt);
		var params = input.params;
		var val = parseFloat(input.value.replace(/,/ig, ""));
		...
		if (params.allowEmpty && isStringEmpty(val))
		{
			return;
		}

		val = FormatUtils.roundTo(val, params.precision);
		val = FormatUtils.formatCurrency(val, params.precision, params.decimalSeparator, params.groupSeparator);
	...
	function keyPressHandler(evt)
	{
		...
		var allowDots = params.precision > 0;
		var allowNegative = params.min < 0;

		if (evt.ctrlKey == true)
			return true;

		if (charCode >= KeyCodes.N0_KEY && charCode <= KeyCodes.N9_KEY)
			return true;

		if (charCode == params.groupSeparator.charCodeAt(0))
		{
			EventHelper.cancel(evt);
			return false;
		}

		if (charCode == params.decimalSeparator.charCodeAt(0))
		{
			if (!allowDots || input.value.indexOf(params.decimalSeparator) != -1)
			{
				EventHelper.cancel(evt);
				return false;
			}
		...
		if (charCode == dash.charCodeAt(0))
		{
			if (!allowNegative || input.value.indexOf(dash) != -1)
			{
				EventHelper.cancel(evt);
				return false;
			}
		...
		if (keyCode == KeyCodes.DELETE_KEY || keyCode == KeyCodes.BACKSPACE_KEY ||
			keyCode == KeyCodes.LEFT_KEY || keyCode == KeyCodes.RIGHT_KEY ||
			keyCode == KeyCodes.HOME_KEY || keyCode == KeyCodes.END_KEY || keyCode == KeyCodes.TAB_KEY)
			return true;
...
function ServiceCallBag()
{
	...
	function onCall()
	{
		var calls = ArrayHelper.groupBy(queue, callID);
		queue = [];
		for (var callID in calls)
		{
			var call = calls[callID];
			var onSuchSuccess = function (response)
			{
				for (var i in call)
				{
					call[i].onSuccess(response);
				}
			};
			var onSuchError = function (response)
			{
				for (var i in call)
				{
					call[i].onError(response);
				}
			};

			var firstCall = call[0];
			firstCall.params.push(onSuchSuccess);
			firstCall.params.push(onSuchError);

			firstCall.action.apply(null, firstCall.params);
		}
	}

	var bagTimeoutHelper = new TimeoutHelper(1000, onCall);

	instance.call = function (callID, action, params, onSuccess, onError)
	{
		queue.push({ callID: callID, action: action, params: params, onSuccess: onSuccess, onError: onError });

		bagTimeoutHelper.call();
	};
}
